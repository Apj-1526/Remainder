...
const Card = ({ data }) => {
  ...
  const handleClick = () => {
    setButton(prevState => prevState === 0 ? 1: 0);
  }

  return (
    <>
      <div className='foarm container h-[150px] w-[190px] mt-3 flex m-auto flex-col justify-around gap-1 rounded-lg border-2 text-white p-5 cursor-pointer hover:border-violet-600 overflow-auto'>
        <h2>work:{data.id}</h2>
        <p>{data.note}</p>
